...
class CardListState extends State<CardList>
  ...
  // onReorderItem callback: newIndex arrives already adjusted for the
  // dragged item's removal at oldIndex.
  Future<void> _handleReorder(int oldIndex, int newIndex) async {
    if (oldIndex == newIndex) return;

    // Reordering is only enabled on the owner's own list outside edit mode,
    // so the displayed list is localInterests itself and the indices match.
    final reordered = List<Interest>.from(localInterests);
    final moved = reordered.removeAt(oldIndex);
    reordered.insert(newIndex, moved);

    // Starred (top 5) interests always stay pinned above the rest; within
    // each group the dragged order is kept.
    final pinned = [
      ...reordered.where((i) => i.favorite),
      ...reordered.where((i) => !i.favorite),
    ];
    ...
    final n = pinned.length;
    final renumbered = [
      for (int i = 0; i < n; i++) pinned[i].copyWith(sort_order: n - i),
    ];
  ...
  Widget build(BuildContext context) {
    ...
    // also guarantees visibleInterests is localInterests itself (no privacy or
    // edit filtering), which _handleReorder relies on for its indices.
        ...
        children: [
          ...
          Expanded(
            child: visibleInterests.isEmpty
                ...
                : _buildInterestList(userModel, visibleInterests, canReorder),
          ),
          if (!widget.signedIn)
            Align(
  ...
  // The scrolling card list. When [canReorder] is true the owner can drag
  // cards to a custom order: long press on touch devices, or the trailing
  // drag handle on desktop and web.
